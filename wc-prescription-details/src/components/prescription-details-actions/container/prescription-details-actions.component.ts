...
export class PrescriptionDetailsActionsComponent {
  ...
  async openAutoAssign(prescription: ReadRequestResource): Promise<void> {
    ...
    try {
      ...
      this.assignCaregivers(prescription.id, prescription.referralTask.id, assignees, uuid, prescription.intent);
  ...
  private assignCaregivers(
    ...
    this._prescriptionStateService.assignMultipleCaregivers(prescriptionId, taskId, assignees, uuid).subscribe({
      next: () => {
        this.loadingActions.set(false);
