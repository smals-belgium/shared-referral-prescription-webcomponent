...
import { IdToken, Intent, LoadingStatus, UserInfo } from '@reuse/code/interfaces';
...
const mockPerformerTask: PerformerTaskResource = {
...
describe('PrescriptionDetailsWebComponent', () => {
  ...
  it('should load a proposals if intent is proposals', async () => {
    ...
    httpMock.expectOne(BASE_URL + '/prescriptions/08e267bf-46e3-459d-8216-d8720acc9f64');
    ...
    httpMock.expectOne(BASE_URL + '/proposals/08e267bf-46e3-459d-8216-d8720acc9f64');
  });
  ...
  it('should request the persons call when user is professional', async () => {
    ...
    const req = httpMock.expectOne(BASE_URL + '/persons/90122712173');
  ...
  it('should display the error card', async () => {
    ...
    const req = httpMock.expectOne(BASE_URL + '/prescriptions/08e267bf-46e3-459d-8216-d8720acc9f64');
  ...
  it('should call loadPrintWebComponent when printer is false', () => {
    createFixture();
    const loadPrintWebComponentSpy = jest.spyOn(
      PrescriptionDetailsWebComponent.prototype as any,
      'loadPrintWebComponent'
    );
  ...
  it('should return the correct border color', () => {
    createFixture();
    let color = component.getStatusBorderColor(RequestStatus.Blacklisted);
    expect(color).toBe('red');
    color = component.getStatusBorderColor(RequestStatus.Cancelled);
    expect(color).toBe('red');
    color = component.getStatusBorderColor(RequestStatus.Expired);
    expect(color).toBe('red');

    color = component.getStatusBorderColor(RequestStatus.Pending);
    expect(color).toBe('orange');

    color = component.getStatusBorderColor(RequestStatus.InProgress);
    expect(color).toBe('#40c4ff');

    color = component.getStatusBorderColor(RequestStatus.Done);
    expect(color).toBe('limegreen');

    color = component.getStatusBorderColor(RequestStatus.Draft);
  ...
  const proposalRequest = (mockResponse: any) => {
    const req = httpMock.expectOne(BASE_URL + '/proposals/08e267bf-46e3-459d-8216-d8720acc9f64');
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
  };
