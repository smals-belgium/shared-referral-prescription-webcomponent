...
export class AutocompleteMultiselectComponent extends EvfBaseFormElementComponent implements OnInit {
  ...
  private setOptionsStream(): void {
    if (this.element.externalSource) {
      ...
      this.options$ = merge(this.queryTrigger$, controlCleared$)
        .pipe(
          debounceTime(300),
          switchMap((value) => value == null ? of(null) : this.externalSourceService
            .handleAutocomplete(this.element.externalSource!, JSON.stringify({
              query: value,
  ...
  private autoSelectOption(options: AutocompleteOption[]) {
    if (this.autoSelectInAutocomplete && options && options.length === 1) {
      ...
      this.announcer.announce(`added ${options[0].label[this.evfTranslate.currentLang]}`);
  ...
  remove(item: AutocompleteOption): void {
    this.selectedItems.update(items => {
      ...
      this.announcer.announce(`Removed ${item.label[this.evfTranslate.currentLang]}`);
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    this.announcer.announce(`added ${option.label[this.evfTranslate.currentLang]}`);
