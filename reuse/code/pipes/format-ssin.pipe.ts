...
export class FormatSsinPipe implements PipeTransform {
  ...
  private static readonly maskSeparators = FormatSsinPipe.mask.replace(/[0-9]/g, '').split('');
  ...
  transform(value?: string): string {
    ...
    return formattedParts.reduce(
      (acc, cur, index) => acc + cur + (index < formattedParts.length - 1 ? FormatSsinPipe.maskSeparators[index] : ''),
      ''
    );
  }
}
