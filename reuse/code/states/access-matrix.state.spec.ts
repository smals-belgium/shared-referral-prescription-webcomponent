...
import { of } from 'rxjs';
import { AccessMatrixState } from './access-matrix.state';
import { AccessMatrixService } from '../services/access-matrix.service';
import { AccessMatrix, LoadingStatus, Permissions } from '../interfaces';
...
describe('AccessMatrixState', () => {
  ...
  it('should call the service to load the access matrix and return the data', (done) => {
    ...
    accessMatrixServiceMock.findForConnectedUser.mockReturnValue(of(mockAccessMatrix));

    accessMatrixState.loadAccessMatrix().subscribe((result) => {
      expect(result).toEqual(mockAccessMatrix);
      expect(accessMatrixServiceMock.findForConnectedUser).toHaveBeenCalled();
      done();
    });
  });
